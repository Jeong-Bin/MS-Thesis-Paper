...
save_name = f"{args.dataset_name.split('_')[0]}2{args.dataset_name2.split('_')[0]}_{args.way}w{args.train_shot}s_{config['save_name_tag']}_{args.kd_mode}KD" #_in{int(args.lambda_inner_ce*10)}{int(args.lambda_inner_kd*10)}_out{int(args.lambda_outer_ce*10)}{int(args.lambda_outer_kd*10)}

import wandb
run = wandb.init(project="TRAIN_OC_cross")
...
def meta_train(args):
    ...
    test_data_transforms = A.Compose([
        ...
        # But when use CIFAR-FS fro cross-domain test, you should resize the CIFAR-FS size.
        A.Resize(84,84),
        A.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), p=1),
        ToTensorV2()
    ])
