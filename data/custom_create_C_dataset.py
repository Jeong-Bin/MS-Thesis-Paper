...
parser.add_argument("--img_size", default=84, type=int, choices=[224, 84],
                                help="Use 84 if the dataset is mini_imagenet. And you don't need this option if the dataset is CUB. Check the variable 'transforms_'")
...
class DistortImageFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # save_path = '/home/hjb3880/WORKPLACE/datasets/imagenet_C' + self.method.__name__ + \
        #             '/' + str(self.severity) + '/' + self.idx_to_class[target]

        save_path = os.path.join(args.new_data_path, args.mode, self.idx_to_class[target] )
        ...
        save_path = save_path_[0] + '_' + self.method.__name__ + '_' + str(self.severity) + '.' + save_path_[1]
...
def glass_blur(x, severity=1):
    ...
    for i in range(c[2]):
        for h in range(args.img_size - c[1], c[1], -1): 
            for w in range(args.img_size - c[1], c[1], -1): 
...
def motion_blur(x, severity=1):
    ...
    if x.shape != (args.img_size, args.img_size):
...
def fog(x, severity=1):
    ...
    x = np.array(x) / 255.
    max_val = x.max()
    x += c[0] * plasma_fractal(wibbledecay=c[1])[:args.img_size, :args.img_size][..., np.newaxis] 
...
def frost(x, severity=1):
    ...
    x_start, y_start = np.random.randint(0, frost.shape[0] - args.img_size), np.random.randint(0, frost.shape[1] - args.img_size)
    frost = frost[x_start:x_start + args.img_size, y_start:y_start + args.img_size][..., [2, 1, 0]]
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(args.img_size, args.img_size, 1) * 1.5 + 0.5)
...
def spatter(x, severity=1):
    ...
    liquid_layer = gaussian(liquid_layer, sigma=c[2], channel_axis=-1)
    ...
    if c[5] == 0:
        ...
        m = gaussian(m.astype(np.float32), sigma=c[4], channel_axis=-1)
        m[m < 0.8] = 0
        #         m = np.abs(m) ** (1/c[4])
...
def pixelate(x, severity=1):
    ...
    x = x.resize((int(args.img_size * c), int(args.img_size * c)), PILImage.BOX)
    x = x.resize((args.img_size, args.img_size), PILImage.BOX)
...
def elastic_transform(image, severity=1):
    c = [(args.img_size * 2, args.img_size * 0.7, args.img_size * 0.1),   # 244 should have been 224, but ultimately nothing is incorrect
         (args.img_size * 2, args.img_size * 0.08, args.img_size * 0.2), 
         (args.img_size * 0.05, args.img_size * 0.01, args.img_size * 0.02),
         (args.img_size * 0.07, args.img_size * 0.01, args.img_size * 0.02),
         (args.img_size * 0.12, args.img_size * 0.01, args.img_size * 0.02)][severity - 1]
    ...
    dx = (gaussian(np.random.uniform(-1, 1, size=shape[:2]),
                   c[1], mode='reflect', truncate=3, channel_axis=-1) * c[0]).astype(np.float32)
...
def save_distorted(method=gaussian_noise):
    ...
    elif args.dataset in ['cub', 'CUB', 'CUB_200_2011']:
        transforms_ = trn.Compose([trn.Resize((args.resize, args.resize))])

    for severity in range(1, 6):
        print(method.__name__, severity)
        distorted_dataset = DistortImageFolder(
                            root = os.path.join(args.origin_data_path, args.mode), 
                            method = method, 
                            severity = severity,
...
elif args.corruptions_num in [4, 19] :
