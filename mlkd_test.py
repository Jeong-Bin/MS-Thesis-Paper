...
from utils.util import seed_fixer, index_preprocessing, knowledge_distillation_loss, soft_CrossEntropy_loss
...
parser.add_argument("--data_dir", default='/home/hjb3880/WORKPLACE/datasets/mini_imagenet_C', type=str)
...
parser.add_argument("--kd_T", default=10, type=float)

parser.add_argument("--teacher_backbone", default='wide_resnet50_2', type=str,
                    choices=['efficientnet_b3', 'tf_efficientnet_b3_ns', 'wide_resnet50_2', 'resnext50_32x4d'])

parser.add_argument("--device", default='cuda', type=str)

parser.add_argument("--teacher_saved_model", default='0715_Train_cub_10w5s_wrn50_outerKD_strong_OC2_10_Teacher', type=str)
parser.add_argument("--student_saved_model", default='0715_Train_cub_10w5s_wrn50_outerKD_strong_OC2_10_Student', type=str)
...
config = {
        'argparse' : args,
        'save_name_tag' : 'wrn50_outerKD_strong_OC2_10', ################################################ _retain_graph
        'readme' : 'kd_T:10, test_adapt_steps:10, valid_tasksets:-1' #, aug 더 강하게
}
...
def fast_adapt(batch, adaptation_indices, evaluation_indices, 
    ...
    for step in range(adaptation_steps):
        ...
        # student_adapt_error = args.ce_alpha*student_adapt_error + args.kd_alpha*kd_loss
...
def meta_test(args):
    ...
    for seed in range(1, 51):
        ...
        # Teacher
        # teacher_backbone = timm.create_model(args.teacher_backbone, features_only=False, pretrained=True, num_classes=args.train_way)
        # a = teacher_backbone.forward_features(torch.randn(1,3,84,84))
        # teacher_backbone.to(device)
        teacher_backbone = None
        # teacher = CNN1(channel_size=a.size(1), kernel_size=a.size(2), num_classes=args.train_way)
